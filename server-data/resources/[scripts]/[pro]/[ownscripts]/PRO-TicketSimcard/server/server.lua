...
QBCore.Functions.CreateUseableItem("adminsimcard", function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('adminsimcard') then return end
    TriggerClientEvent("adminsimcard:UseSimcard", source)
end)

RegisterNetEvent('checkNumberr', function(item)
    local src = source
    ...
    if result2 == 0 and result == 0 then
        ...
        Player.Functions.RemoveItem('adminsimcard', 1)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["adminsimcard"], "remove")
    else
        TriggerClientEvent('notify2', src)
    end
end)

RegisterNetEvent('checkNumberrrand', function()
