...
RegisterNetEvent('pro-trashsearch:server:searchTrash', function(data)
    local src = source
    ...
    TriggerClientEvent('pro-trashsearch:client:searchTrash', src, binId)
end)
...
-- Register NetEvent pro-trashsearch:server:searchedTrash
RegisterNetEvent('pro-trashsearch:server:searchedTrash', function(binId)
    local src = source

    -- Check if player is legit
    if not isLegit(src) then
        TriggerClientEvent("qb-anticheat:client:ban", src, "pro-trashsearch -> Player " .. GetPlayerName(src) .. " tried to bypass cooldown")
        error("pro-trashsearch -> Player tried to bypass cooldown")
    end
