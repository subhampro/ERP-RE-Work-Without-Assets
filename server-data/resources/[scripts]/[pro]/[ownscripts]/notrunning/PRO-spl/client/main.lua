    ...
    CreateThread(function()       
        ...
        SetTimeout(Config.WaitTime * (60 * 1000), function()
            ...
            if IsEntityDead(pilot) then 
                QBCore.Functions.Notify("The plane was destroyed!", "error", 6500)
                return
            end
            ...
            QBCore.Functions.Notify("Drop Off done, keep your eyes on the sky!","success", 6500)   
            ...
            exports['qb-target']:AddTargetEntity(crate, {
                options = {
                    {
                        type = 'client',
                        event = 'qb-wepdrope:client:getItems',
                        label = 'Take Weapons',
                        icon = 'fas fa-hand',
                    }
                },
...
-- Create the AirDrop
RegisterNetEvent("qb-wepdrope:client:CreateDrop", function(roofCheck, planeSpawnDistance)
    if canDrop then
        ...
        QBCore.Functions.Progressbar('call_wepdrop', 'Making the request...', 8000, false, true, {
            ...
            QBCore.Functions.TriggerCallback("qb-wepdrope:server:getCops", function(CurrentCops)
                if CurrentCops >= Config.RequiredCops then
                    TriggerEvent("qb-wepdrope:client:StartDrop", roofCheck or false, planeSpawnDistance or 400.0, {["x"] = playerCoords.x, ["y"] = playerCoords.y, ["z"] = playerCoords.z})
                    
                    -- You could trigger a police alert here

                    TriggerServerEvent('QBCore:Server:RemoveItem', 'dropphonee', 1)
                    TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items['dropphonee'], 'remove')
                    ...
                    QBCore.Functions.Notify("The pilot is currently busy. Try again Later", "error")
                end    
            end)
        end, function()
...
-- Start the AirDrop
RegisterNetEvent("qb-wepdrope:client:StartDrop", function(roofCheck, planeSpawnDistance, dropCoords)
    -- TriggerServerEvent('police:server:policeAlert', 'Its seems some illigal Drop overthere!')
    CreateThread(function()          
...
RegisterNetEvent('qb-wepdrope:client:getItems', function()
    -- TriggerServerEvent('police:server:policeAlert', 'Illigal Crate Opening Going On!')
    QBCore.Functions.Progressbar('open_crate', 'Opening the crate...', 100, false, true, {
        ...
        TriggerServerEvent('qb-wepdrope:server:recieveItems')
        DeleteEntity(crate)
...
CreateThread(function()
    ...
    exports['qb-target']:AddTargetEntity(dropPed, {
        options = {
            {
                type = 'server',
                event = 'qb-wepdrope:server:getDropPhone',
