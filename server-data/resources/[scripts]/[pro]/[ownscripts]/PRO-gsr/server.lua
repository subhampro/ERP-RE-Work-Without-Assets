...
QBCore.Commands.Add("gsr", "GSR Test for Police ", {{name ="id ", help="Player ID"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    -- local xPlayer = QBCore.Functions.GetPlayer(src)
    local number = tonumber(args[1])
    if args[1] ~= nil then 
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "sasp" or Player.PlayerData.job.name == "bcso"  then
		-- if xPlayer.PlayerData.job.name == 'police' and type(number) == "number" then
            ...
            TriggerClientEvent("QBCore:Notify", src, 'You must be a Police Wala to use the Gun Powder test', "error", 5000)
    	end
	else
