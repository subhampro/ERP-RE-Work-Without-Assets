...
--
		if dist <= 0.5 then
			...
			TriggerServerEvent("PRO-chickenjob:startChicken")
			LapChicken()
			end			
		end
	end
...
if share == true then
	...
	andsplashed = 0
	share = false
	exports['Roda_Notifications']:showNotify('Take Alived Chiken To Process Area ..', 'info', 6000)
	TriggerServerEvent("PRO-chickenjob:getNewChicken")
	end
...
function hewassplashed()
	...
	if chanceofsplashes <= 60 then
		    exports['Roda_Notifications']:showNotify('You managed to catch 1 chicken!', 'success', 6000)
			if Caught1 == 1 then
		...
		exports['Roda_Notifications']:showNotify("The chicken escaped your arms!", 'error', 6000)
	end
end
-------------------DEVELOPED BY MrRobot-----------------------------------------
...
function packingg(stanowisko)
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			QBCore.Functions.Progressbar("wash-", "Packing..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
			TriggerServerEvent("PRO-chickenjob:getpackedChicken",2)
			exports['Roda_Notifications']:showNotify("Keep packing the chicken or go to the vehicle and store it.", 'info', 6000)
		...
		exports['Roda_Notifications']:showNotify("You have nothing to pack!", 'error', 6000)
		end
...
function packed(stanowisko)
	...
	LoadDict('anim@heists@box_carry@')
		...
		exports['Roda_Notifications']:showNotify("You stopped packing!", 'error', 6000)
...
function portioning(position)
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			QBCore.Functions.Progressbar("Cut-", "Slaughtering..", 20000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			...
			exports['Roda_Notifications']:showNotify("Now Pack slaughtered chicken!", 'info', 6000)	
			TriggerServerEvent("PRO-chickenjob:getcutChicken", 1)
			FreezeEntityPosition(GetPlayerPed(-1),false)
			...
			exports['Roda_Notifications']:showNotify("You dont have any chickens!", 'error', 6000)	
		end
	end, 'alivechicken')
end
...
function packedsell()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
		if result then
			...
			QBCore.Functions.Progressbar("Cut-", "Selling..", 15000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
			}, {}, {}, {}, function()
				TriggerServerEvent("PRO-chickenjob:sell",3)
				ClearPedTasks(GetPlayerPed(-1))
				DeleteEntity(prop)
			end, function() -- Cancel
			...
			exports['Roda_Notifications']:showNotify("You have nothing to sell!", 'error', 6000)	
		end
