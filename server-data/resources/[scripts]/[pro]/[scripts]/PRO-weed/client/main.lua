...
-- 				TriggerServerEvent('PRO-weed:processCannabis')
-- 				-- 	if result then
-- 				-- 		ProcessWeed()
-- 				-- 	else
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				QBCore.Functions.TriggerCallback('weed:ingredient', function(HasItem, type)
					if HasItem then
						...
						QBCore.Functions.Notify('You dont have enough Materials for Packed Weed 2og', 'error')
					end
				end)
			end
		else
			...
			if IsControlJustReleased(0, 38) and not isProcessing then
				QBCore.Functions.TriggerCallback('weed:ingredient2', function(HasItem, type)
					if HasItem then
						...
						QBCore.Functions.Notify('You dont have enough Materials For Weed 4og', 'error')
					end
				end)
			end
		else
	...
	QBCore.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		while timeLeft > 0 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.WeedProcessing.coords, false) > 4 then
				TriggerServerEvent('PRO-weed:cancelProcessing')
				break
			end
		end
...
function ProcessWeed2()
	...
	QBCore.Functions.Progressbar("search_register", "Processing..", 15000, false, true, {
		...
		TriggerServerEvent("PRO-weed:server:process")
		--TriggerServerEvent('PRO-weed:processCannabis2')
		local timeLeft = Config.Delays.WeedProcessing / 1000
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				QBCore.Functions.Notify("Plucking Cannabis", "error", 10000)
				QBCore.Functions.Progressbar("search_register", "Picking up Cannabis..", 10000, false, true, {
					...
					TriggerServerEvent('PRO-weed:pickedUpCannabis')
					TriggerServerEvent('PRO-weed:weed')
...
function SpawnWeedPlants()
    ...
    while b < random do
        ...
        local E = CreateObject(-305885281, D.x + math.random(-25,25), D.y + math.random(-25,25), D.z+116, false, false, true)
