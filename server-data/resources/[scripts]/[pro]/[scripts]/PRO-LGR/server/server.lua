...
AddEventHandler('qb-log:server:CreateLog', function(name, title, color, message, tagEveryone)
    ...
    PerformHttpRequest(webHook, function(err, text, headers) end, 'POST', json.encode({ username = "SubhamPRO Logs",embeds = embedData}), { ['Content-Type'] = 'application/json' })
    Citizen.Wait(100)
    if tag then
        PerformHttpRequest(webHook, function(err, text, headers) end, 'POST', json.encode({ username = "SubhamPRO Logs ", content = "@everyone"}), { ['Content-Type'] = 'application/json' })
    end
end)
...
QBCore.Commands.Add("testwebhook", ":)", {}, false, function(source, args)
    ...
    TriggerEvent("qb-log:server:CreateLog", "police", "OFF DUTY", "default", "Name : "..player.PlayerData.charinfo.firstname..' '..player.PlayerData.charinfo.lastname..'\nRank : '..player.PlayerData.job.gradelabel)
end, "god")
