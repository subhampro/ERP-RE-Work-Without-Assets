...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("PRO-boombox:joined")
end)
...
RegisterNUICallback("loadServer",function(data, cb)
	...
	local tempTable = {
		...
		TriggerServerEvent("PRO-boombox:loadSpeaker", tempTable[1])
end)


RegisterNetEvent("PRO-boombox:loadSpeakerClient")
AddEventHandler("PRO-boombox:loadSpeakerClient", function(speaker)
	cacheSpeaker[speaker.speaker] = speaker
end)


RegisterNetEvent("PRO-boombox:joined")
AddEventHandler("PRO-boombox:joined", function(speaker)
	cacheSpeaker = speaker
end)

RegisterNetEvent("PRO-boombox:removeClient")
AddEventHandler("PRO-boombox:removeClient", function(speaker)
...
RegisterNetEvent("PRO-boombox:switchVideoClient")
AddEventHandler("PRO-boombox:switchVideoClient", function(id, videoStatus, time)
	cacheSpeaker[id].switch = true
	cacheSpeaker[id].videoStatus = videoStatus
	cacheSpeaker[id].time = time - cacheSpeaker[id].time
end)

RegisterNetEvent("PRO-boombox:changeVolClient")
AddEventHandler("PRO-boombox:changeVolClient", function(id, vol)
                ...
                if v.id ~= nil then
					...
					if dist < 20.0 then
						...
						elseif playing == k and onSwitch then
							...
							TriggerServerEvent("PRO-boombox:switchVideo", k, videoStatus, newTime)
						...
						elseif playing == k and onVolChange then
							...
							TriggerServerEvent("PRO-boombox:changeVol", k, volchange)
			...
			for k, v in pairs(cacheSpeaker) do
				...
				if #(v.coords - GetEntityCoords(PlayerPedId())) < 2.0 then
					...
					if IsControlJustPressed(0, 47) then
						...
						TriggerServerEvent("PRO-boombox:removeSpeaker", currentId)
...
RegisterNetEvent("PRO-boombox:place")
AddEventHandler("PRO-boombox:place", function()
	local plyrcoords, forward = GetEntityCoords(PlayerPedId()), GetEntityForwardVector(PlayerPedId())
	local spawncoords = (plyrcoords + forward * 1.0)
	tooClose = false
	if #cacheSpeaker > 0 then
		...
		if not tooClose then
			...
			TriggerServerEvent("PRO-boombox:placedSpeaker", spawncoords, speaker)
			SetEntityHeading(speaker, GetEntityHeading(PlayerPedId()))
		else
			QBCore.ShowNotification(Config["translations"].tooClose, false, false)
		end
	else
		speaker = CreateObject(1729911864, spawncoords, true, true, true)
		FreezeEntityPosition(speaker, true)
