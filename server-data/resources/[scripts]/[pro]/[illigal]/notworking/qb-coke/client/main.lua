			...
			if IsControlJustPressed(1, 51) then		
				...
				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						QBCore.Functions.Notify("Some Item Missing to Process Brick", "error")
						isProcessing = false
					end
				end, 'empty_coke_bag')
			end
...
function processing()
	...
	SetEntityHeading(player, 212.28)
	FreezeEntityPosition(player, true)
	playAnim("anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 15000)

	QBCore.Functions.Progressbar("coke-", "Try to Breaking down the coke..", 15000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
