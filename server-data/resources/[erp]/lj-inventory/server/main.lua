...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif itemData.name == 'smallbackpack' then
			if Player.Functions.RemoveMoney("cash", 20000, "smallbackpack") then
				local info = {
					smallbackpackid = math.random(1111,9999),
				}
				Player.Functions.AddItem('smallbackpack', 1, nil, info, {["quality"] = 200})
			else
				TriggerClientEvent('QBCore:Notify', src, "You don't have sufficient cash balance!", "error")
			end
		elseif itemData.name == 'mediumbackpack' then
			if Player.Functions.RemoveMoney("cash", 30000, "mediumbackpack") then
				local info = {
					mediumbackpackid = math.random(1111,9999),
				}
				Player.Functions.AddItem('mediumbackpack', 1, nil, info, {["quality"] = 250})
			else
				TriggerClientEvent('QBCore:Notify', src, "You don't have sufficient cash balance!", "error")
			end
		elseif itemData.name == 'largebackpack' then
			if Player.Functions.RemoveMoney("cash", 40000, "largebackpack") then
				local info = {
					largebackpackid = math.random(1111,9999),
				}
			Player.Functions.AddItem('largebackpack', 1, nil, info, {["quality"] = 300})
			else
				TriggerClientEvent('QBCore:Notify', src, "You don't have sufficient cash balance!", "error")
			end
...
QBCore.Functions.CreateUseableItem("driver_license", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		for k,v in pairs(Players) do
			...
			if dist < 3 then
				TriggerClientEvent('qb-idcard:client:driveropen', v, csn, name, name2, birth, gender, national, photo)
			end
		end
		TriggerClientEvent('qb-idcard:client:driveropen', src, csn, name, name2, birth, gender, national, photo)
	end	
end)
...
QBCore.Functions.CreateUseableItem("id_card", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		for k,v in pairs(Players) do
			...
			local dist = #( GetEntityCoords(ply) - GetEntityCoords(target) )
			if dist < 3 then
				TriggerClientEvent('qb-idcard:client:open', v, csn, name, name2, birth, gender, national, photo)
			end
		end
		TriggerClientEvent('qb-idcard:client:open', src, csn, name, name2, birth, gender, national, photo)
	end
end)
...
QBCore.Functions.CreateUseableItem("weaponlicense", function(source, item)
		...
		if Player.Functions.GetItemBySlot(item.slot) ~= nil then
			for k,v in pairs(Players) do
				...
				local dist = #( GetEntityCoords(ply) - GetEntityCoords(target) )
				if dist < 3 then
					TriggerClientEvent('pro-idcard:client:Weaponopen', v, csn, name, name2, birth, gender, national, photo)
				end
			end
			TriggerClientEvent('pro-idcard:client:Weaponopen', src, csn, name, name2, birth, gender, national, photo)
		end	
	end)	
