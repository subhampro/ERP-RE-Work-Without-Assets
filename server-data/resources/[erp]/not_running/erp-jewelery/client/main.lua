...
local function smashVitrine(k)
    if validWeapon() then
        ...
        elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
            exports['Roda_Notifications']:showNotify("You've left a fingerprint on the glass", 'error', 6000)
        end
        smashing = true
        QBCore.Functions.Progressbar("smash_vitrine", "Looting case", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
            ...
            TriggerServerEvent('erp-jewelery:server:setVitrineState', "isOpened", true, k)
            TriggerServerEvent('erp-jewelery:server:setVitrineState', "isBusy", false, k)
            TriggerServerEvent('erp-jewelery:server:vitrineReward')
            TriggerServerEvent('erp-jewelery:server:setTimeout')
        ...
        TriggerServerEvent('erp-jewelery:server:setVitrineState', "isBusy", true, k)
        ...
        exports['Roda_Notifications']:showNotify("Your weapon does not seem strong enough..", 'error', 6000)
    end
end
...
local ThermiteEffect = function()
    ...
    Wait(1500)
    TriggerServerEvent("erp-jewelery:server:ThermitePtfx")
    Wait(500)
...
-- Events
RegisterNetEvent('erp-jewelery:client:Thermite', function()
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
        if result then 
            if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
                TriggerServerEvent("evidence:server:CreateFingerDrop", GetEntityCoords(PlayerPedId()))
            end
            QBCore.Functions.TriggerCallback('erp-jewelery:server:getCops', function(cops)
                if cops >= Config.RequiredCops then
                    ...
                    function()
                        exports['Roda_Notifications']:showNotify("Thermite failed..", 'error', 6000)
                    end)
                else
                    exports['Roda_Notifications']:showNotify("Not enough police..", 'error', 6000)
                end
            end)
        else
            exports['Roda_Notifications']:showNotify("You are missing something(s)..", 'error', 6000)
        end
    end, {"thermite", "lighter"})
end)

RegisterNetEvent('erp-jewelery:client:ThermitePtfx', function()
...
RegisterNetEvent("erp-jewelery:client:ThermitePtfx", function(coords)
...
RegisterNetEvent('erp-jewelery:client:setVitrineState', function(stateType, state, k)
...
CreateThread(function()
    ...
    exports['qb-target']:AddBoxZone("JewelleryThermite", vector3(-595.94, -283.74, 50.32), 0.4, 0.8, {
        ...
        maxZ = 51.35,
            ...
            {
                type = "client",
                event = "erp-jewelery:client:Thermite",
                icon = 'fas fa-gem',
