...
Citizen.CreateThread(function()
	while true do
		...
		if not IsMenuOpen() then
			QBCore.Functions.TriggerCallback('qb-tattoos:GetPlayerTattoos', function(tattooList)
				if tattooList then
...
function BuyTattoo(collection, name, label, price)
	QBCore.Functions.TriggerCallback('qb-tattoos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("qb-tattoos:RemoveTattoo", currentTattoos)
	QBCore.Functions.Notify("You have tattoo "..GetLabelText(label).. " deleted.", "error", 2500)
end
...
RegisterNetEvent('ctrp-tattoo:client:tattooppen')
AddEventHandler('ctrp-tattoo:client:tattooppen', function()
    OpenTattooShop()
end)
