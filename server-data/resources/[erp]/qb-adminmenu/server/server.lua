...
RegisterNetEvent('qb-admin:server:revive', function(player)
    ...
    TriggerClientEvent('hospital:client:SetArmor', 100, src)
end)
...
QBCore.Commands.Add('getkeys', Lang:t("commands.save_vehicle_keys"), {}, false, function(source, _)
...
QBCore.Commands.Add('reportreply', Lang:t("commands.reply_to_report"), {{name='id', help='Player'}, {name = 'message', help = 'Message to respond with'}}, false, function(source, args)
    local src = source
    local playerId = tonumber(args[1])
    table.remove(args, 1)
    local msg = table.concat(args, ' ')
    ...
    if msg == '' then return end
    if not OtherPlayer then return TriggerClientEvent('QBCore:Notify', src, 'Player is not online', 'error') end
    if not QBCore.Functions.HasPermission(src, 'rr') or IsPlayerAceAllowed(src, 'command') ~= 1 then return end
    TriggerClientEvent('chat:addMessage', playerId, {
        color = {255, 0, 0},
        multiline = true,
        args = {'Admin Response', msg}
