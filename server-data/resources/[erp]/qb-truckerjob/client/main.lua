        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                elseif type == "stores" then
                    ...
                    exports['Roda_Notifications']:showNotify(Lang:t("mission.store_reached"), 'success', 6000)
...
local function getNewLocation()
    ...
    if location ~= 0 then
        ...
        exports['Roda_Notifications']:showNotify(Lang:t("success.payslip_time"), 'success', 6000)
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
	        ClearAllBlipRoutes()
            CurrentBlip = nil
    ...
    if IsPedInAnyVehicle(PlayerPedId()) and isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
            if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                ...
                exports['Roda_Notifications']:showNotify(Lang:t("error.vehicle_not_correct"), 'error', 6000)
            end
        else
            exports['Roda_Notifications']:showNotify(Lang:t("error.no_driver"), 'error', 6000)
        end
    else
        MenuGarage()
    end
...
RegisterNetEvent('qb-truckerjob:client:PaySlip', function()
    if JobsDone > 0 then
        ...
        exports['Roda_Notifications']:showNotify(Lang:t("error.no_work_done"), 'error', 6000)
    end
        ...
        if isTruckerVehicle(vehicle) and CurrentPlate == QBCore.Functions.GetPlate(vehicle) then
            ...
            if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                ...
                QBCore.Functions.Progressbar("work_carrybox", Lang:t("mission.take_box"), 2000, false, true, {
                    ...
                    isWorking = false
                    StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                    exports['Roda_Notifications']:showNotify(Lang:t("error.cancelled"), 'error', 6000)
                end)
            end
        end
    end
...
RegisterNetEvent('qb-trucker:client:Deliver', function()
    ...
    QBCore.Functions.Progressbar("work_dropbox", Lang:t("mission.deliver_box"), 2000, false, true, {
        ...
        if currentCount == CurrentLocation.dropcount then
            ...
            TriggerServerEvent("jim-shops:server:RestockShopItems", CurrentLocation.store)
            exports['Roda_Notifications']:showNotify(Lang:t("mission.goto_next_point"), 'info', 6000)
            ...
            exports['Roda_Notifications']:showNotify(Lang:t("mission.another_box"), 'info', 6000)
        end
    end, function() -- Cancel
        isWorking = false
        ClearPedTasks(PlayerPedId())
