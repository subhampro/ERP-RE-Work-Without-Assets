                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerEvent("qb-log:server:CreateLog", "storerobbery", "Try to do Store Robbery", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) ")
                                        copsCalled = true
                                    end
                                else
                                    exports['Roda_Notifications']:showNotify("Not Enough Police (".. Config.MinimumStoreRobberyPolice .." Required)", 'error', 6000)
                                end
                            end
                        else
    ...
    for k in pairs(Config.Registers) do
        ...
        if dist <= 1 and not Config.Registers[k].robbed then
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                ...
                exports['Roda_Notifications']:showNotify("Not Enough Police (2 Required)", 'error', 6000)
            end
        end
    end
end)
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        ...
        QBCore.Functions.Progressbar("search_register", "Emptying The Register..", lockpickTime, false, true, {
            ...
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            exports['Roda_Notifications']:showNotify("Process canceled..", 'error', 6000)
            currentRegister = 0
        end)
        CreateThread(function()
...
RegisterNUICallback('fail', function(_ ,cb)
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        exports['Roda_Notifications']:showNotify("You Broke The Lock Pick", 'error', 6000)
    end
    lockpick(false)
    cb('ok')
end)
