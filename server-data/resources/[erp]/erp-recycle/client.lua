...
CreateThread(function()
	if Config.RequireJob then
		...
		{ options = { { event = "erp-recycle:EnterTradeWarehouse", icon = "fas fa-recycle", label = "Enter Warehouse", job = Config.Job }, },
						distance = 1.5 })
	else
		Targets["RecyclingEnter"] =
		exports['qb-target']:AddBoxZone("RecyclingEnter", vector3(746.82, -1398.93, 26.55), 0.4, 1.6, { name="RecyclingEnter", debugPoly=Config.Debug, minZ=25.2, maxZ=28.0 },
		{ options = { { event = "erp-recycle:EnterTradeWarehouse", icon = "fas fa-recycle", label = "Enter Warehouse", }, },
						distance = 1.5 })
	end
    ...
    { options = { { event = "erp-recycle:ExitTradeWarehouse", icon = "fas fa-recycle", label = "Exit Warehouse", }, },
					distance = 1.5 })
	Targets["recycleduty"] =
    ...
    { options = { { event = "erp-recycle:dutytoggle", icon = "fas fa-hard-hat", label = "Toggle Recycling Duty", }, },
					distance = 1.5 })
	Targets["tradeitems"] =
    ...
    { options = { { event = "erp-recycle:Trade:Menu", icon = "fas fa-box", label = "Trade Materials", }, },
					distance = 1.5 })
end)

local carryPackage = nil
RegisterNetEvent('erp-recycle:EnterTradeWarehouse', function()
	if Config.EnableOpeningHours then
		...
		if ClockTime >= Config.OpenHour and ClockTime <= Config.CloseHour - 1 then
			if (ClockTime >= Config.OpenHour and ClockTime < 24) or (ClockTime <= Config.CloseHour -1 and ClockTime > 0) then
				...
				exports['Roda_Notifications']:showNotify("We're currently closed, we're open from "..Config.OpenHour..":00am till "..Config.CloseHour..":00pm", 'error', 6000)
			end
		else
			exports['Roda_Notifications']:showNotify("We're currently closed, we're open from 9:00am till 21:00pm", 'error', 6000)
		end
	else
		renderPropsWareHouse()
		DoScreenFadeOut(500)
...
RegisterNetEvent('erp-recycle:ExitTradeWarehouse', function()
	for k, v in pairs(props) do DeleteObject(props[k]) end props = {} 
	DoScreenFadeOut(500)
	while not IsScreenFadedOut() do
		Citizen.Wait(10)
	end
	if onDuty then TriggerEvent('erp-recycle:dutytoggle') end
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    ...
                    elseif #(vector3(pos.x, pos.y, pos.z) - packagePos) < 100 then
                        DrawText3D(packagePos.x, packagePos.y, packagePos.z + 1, "Pickup Here")
                    end
                else
                    if #(vector3(pos.x, pos.y, pos.z) - vector3(Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z)) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Packing into recyclable box..", 5000, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = true, }, 
                                ...
                                TriggerServerEvent('erp-recycle:getrecyclablematerial')
                                GetRandomPackage()
                            end)
                        end
                    else
                        DrawText3D(Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, "Transfer Here")
                    end
                end
            else
                GetRandomPackage()
...
--Third Eye Commands
RegisterNetEvent('erp-recycle:dutytoggle', function()
	if Config.RequireJob then
		TriggerServerEvent("QBCore:ToggleDuty")
	else
		onDuty = not onDuty
		if onDuty then exports['Roda_Notifications']:showNotify('You went on duty', 'success', 6000)
		else exports['Roda_Notifications']:showNotify('You went off duty', 'success', 6000) end
	end
end)

RegisterNetEvent("erp-recycle:CloseMenu", function() exports['qb-menu']:closeMenu() end)


--Trade Anim small 
RegisterNetEvent('erp-recycle:TradeAnim', function(data)
	local pid = PlayerPedId()
	loadAnimDict("mp_common")
	TriggerServerEvent('erp-recycle:TradeItems', data) -- Had to slip in the sell command during the animation command
...
--Recyclable Trader Menu
RegisterNetEvent('erp-recycle:Trade:Menu', function()
    exports['qb-menu']:openMenu({
		{ header = "Material Trading", txt = "Trade collected materials", isMenuHeader = true },
 		{ header = "", txt = "✘ Close", params = { event = "erp-recycle:CloseMenu" } },
		{ header = "Trade 10 Materials", params = { event = "erp-recycle:TradeAnim", args = 1 } },
		{ header = "Trade 100 Materials", params = { event = "erp-recycle:TradeAnim", args = 2 } },
    })
end)
