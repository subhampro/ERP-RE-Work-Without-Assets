...
RegisterCommand("help", function(source, args, raw)
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) and spam then
		QBCore.Functions.TriggerCallback('erp-ai_medic:docOnline', function(EMSOnline, hasEnoughMoney)
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				SpawnVehicle(GetEntityCoords(PlayerPedId()))
				exports['Roda_Notifications']:showNotify('Your medic is on the way', 'success', 6000)
			else
				if EMSOnline > Config.Doctor then
					exports['Roda_Notifications']:showNotify("There are too many EMT's online", 'error', 6000)
				elseif not hasEnoughMoney then
					exports['Roda_Notifications']:showNotify("Not Enough Money", 'error', 6000)
				else
					exports['Roda_Notifications']:showNotify("Wait EMT's are on the Way", 'success', 6000)
				end	
			end
		end)
	else
		exports['Roda_Notifications']:showNotify("This can only be used in last stand", 'error', 6000)
	end
end)
...
function SpawnVehicle(x, y, z)  
	...
	if not DoesEntityExist(vehhash) then
		...
		SetVehicleNumberPlateText(mechVeh, "erp-ai_medic")
...
function DoctorNPC()
	...
	QBCore.Functions.Progressbar("revive_doc", "Checking Your Status", Config.ReviveTime, false, false, {
		...
		TriggerServerEvent('erp-ai_medic:charge')
		-- TaskEnterVehicle(driverPed,ambulanceVehicle,-1,0,1.0,1)
		
		local tempInd=1
		local tempDis=10000000000
		for index, value in ipairs(Config.hospitalLocations) do
		...
		Wait(200)
		local hospitalLocation =Config.hospitalLocations[tempInd]
