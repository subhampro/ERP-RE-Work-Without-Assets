    ...
    while true do
        ...
        if dist < 20 then
            ...
            if #(pos - vector3(Config.Cityhall.coords.x, Config.Cityhall.coords.y, Config.Cityhall.coords.z)) < 1.5 then
                DrawText3Ds(Config.Cityhall.coords, '~g~E~w~ - City Hall Menu')
...
RegisterNUICallback('requestId', function(data)
    if inRange then
        ...
        exports['Roda_Notifications']:showNotify('You have recived your '..idTypes[idType].label..' for $50', 'success', 7000)
    else
        exports['Roda_Notifications']:showNotify('This will not work', 'error', 7000)
    end
end)
...
RegisterNUICallback('applyJob', function(data)
    if inRange then
        ...
        exports['Roda_Notifications']:showNotify('Unfortunately will not work ...', 'error', 7000)
    end
end)
