            ...
            if Distance < 15 or hasBag then
                ...
                if not hasBag and canTakeBag then
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            ...
                            if IsControlJustPressed(0, 51) and hasBag then
                                QBCore.Functions.Progressbar("deliverbag", Lang:t("info.progressbar"), 2000, false, true, {
                                    ...
                                    if (amountOfBags - 1) <= 0 then
                                        QBCore.Functions.TriggerCallback('garbagejob:server:NextStop', function(hasMoreStops, nextStop, newBagAmount)
                                            if hasMoreStops and nextStop ~= 0 then
                                                ...
                                                SetGarbageRoute()
                                                exports['Roda_Notifications']:showNotify(Lang:t("info.all_bags"), "info", 6000)
                                                listen = false
                                            else
                                                if hasMoreStops and nextStop == currentStop then
                                                    exports['Roda_Notifications']:showNotify(Lang:t("info.depot_issue"), "info", 6000)
                                                    amountOfBags = 0
                                                else
                                                    -- You are done with work here.
                                                    exports['Roda_Notifications']:showNotify(Lang:t("info.done_working"), "info", 6000)
                                                    RemoveBlip(deliveryBlip)
                                                    SetRouteBack()
                                                    amountOfBags = 0
                                                    listen = false
                                        ...
                                        if amountOfBags > 1 then
                                            exports['Roda_Notifications']:showNotify(Lang:t("info.bags_left", {value = amountOfBags}), "info", 6000)
                                        else
                                            exports['Roda_Notifications']:showNotify(Lang:t("info.bags_still", {value = amountOfBags}), "info", 6000)
                                        end
                                        hasBag = false
                                    end
                                    ...
                                    exports['Roda_Notifications']:showNotify(Lang:t("error.cancled"), "error", 6000)
                                end)
                        ...
                        exports['Roda_Notifications']:showNotify(Lang:t("error.no_truck"), "error", 6000)
                        DeliverAnim()
                        hasBag = false
                    end
                end
    ...
    QBCore.Functions.TriggerCallback('garbagejob:server:NewShift', function(shouldContinue, firstStop, totalBags)
        if shouldContinue then
            ...
            QBCore.Functions.SpawnVehicle("trash2", function(veh)
                ...
                SetGarbageRoute()
                exports['Roda_Notifications']:showNotify(Lang:t("info.deposit_paid", {value = Config.TruckPrice}), "info", 6000)
                exports['Roda_Notifications']:showNotify(Lang:t("info.started"), "info", 6000)
            end, coords, true)
        else
            exports['Roda_Notifications']:showNotify(Lang:t("info.not_enough", {value = Config.TruckPrice}), "info", 6000)
        end
    end)
end)
...
RegisterNetEvent('qb-garbagejob:client:RequestPaycheck', function()
    if garbageVehicle then
        BringBackCar()
        exports['Roda_Notifications']:showNotify(Lang:t("info.truck_returned"), "info", 6000)
    end
