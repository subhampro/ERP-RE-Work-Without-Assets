		...
		if k == "vendingmachine" and Config.VendOverride then
			...
			for l, b in pairs(v["coords"]) do
				if not v["hideblip"] then
					...
					SetBlipScale(StoreBlip, v["blipsize"])
				...
				if Config.Peds then
					if v["model"] then
						...
						if IsModelAPed(v["model"]) then
							if not ped["Shop - ['"..k.."("..l..")']"] then ped["Shop - ['"..k.."("..l..")']"] = CreatePed(0, v["model"], b.x, b.y, b.z-1.0, b.a, false, false) end
							if not v["killable"] then SetEntityInvincible(ped["Shop - ['"..k.."("..l..")']"], true) end
							TaskStartScenarioInPlace(ped["Shop - ['"..k.."("..l..")']"], v["scenario"])
							SetBlockingOfNonTemporaryEvents(ped["Shop - ['"..k.."("..l..")']"], true)
							SetEntityNoCollisionEntity(ped["Shop - ['"..k.."("..l..")']"], PlayerPedId(), false)
						end
						if not IsModelAPed(v["model"]) then
							if not ped["Shop - ['"..k.."("..l..")']"] then
								ped["Shop - ['"..k.."("..l..")']"] = CreateObject(v["model"], b.x, b.y, b.z-1.03, 0, 0, 0)
								SetEntityHeading(ped["Shop - ['"..k.."("..l..")']"], b.w)
							end
						end
						FreezeEntityPosition(ped["Shop - ['"..k.."("..l..")']"], true)
...
RegisterNetEvent('jim-shops:ShopMenu', function(data, custom)
	...
	for i = 1, #products do
		...
		if not QBCore.Shared.Items[products[i].name:lower()] then
            ...
            setheader = QBCore.Shared.Items[products[i].name].label 
...
RegisterNetEvent('jim-shops:Charge', function(data)
	...
	if dialog then
		...
		if Config.Limit and data.custom == nil then	if tonumber(dialog.amount) > tonumber(data.amount) then exports['Roda_Notifications']:showNotify("Incorrect amount", "error", 6000) TriggerEvent("jim-shops:Charge", data) return end end
		if tonumber(dialog.amount) <= 0 then exports['Roda_Notifications']:showNotify("Incorrect amount", "error", 6000) TriggerEvent("jim-shops:Charge", data) return end
