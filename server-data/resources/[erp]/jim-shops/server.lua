...
RegisterServerEvent('jim-shops:GetItem', function(amount, billtype, item, shoptable, price, info, shop, num, nostash)
	...
	if (totalWeight + (QBCore.Shared.Items[item].weight * amount)) > maxWeight then
		TriggerClientEvent('Roda_Notifications:showNotification', src, "Not enough space in inventory", 'error', 6000)
	-- If unique and it would poof away:
	elseif QBCore.Shared.Items[item].unique and (tonumber(slots) < tonumber(amount)) then
		TriggerClientEvent('Roda_Notifications:showNotification', src, "Not enough slots in inventory", 'error', 6000)
	else
		--Money Check
		if balance <= (tonumber(price) * tonumber(amount)) then -- Check for money first if not enough, stop here
			TriggerClientEvent('Roda_Notifications:showNotification', src, "Not enough money", 'error', 6000) return
		end
		...
		if QBCore.Shared.Items[item].type == "weapon" or QBCore.Shared.Items[item].unique then
			...
			for i = 1, amount do -- Make a loop to put items into different slots rather than full amount in 1 slot
				if Player.Functions.AddItem(item, 1, nil, info) then
					...
					TriggerClientEvent('Roda_Notifications:showNotification', src, "Can't give item!", 'error', 6000) break -- stop the item giving loop
				end
				Wait(5)
			end
		else
			...
			if Player.Functions.AddItem(item, amount, nil, info) then
				...
				TriggerClientEvent('Roda_Notifications:showNotification', src, "Can't give item!", 'error', 6000)
			end
		end
...
RegisterNetEvent("jim-shops:server:RestockShopItems", function(storeinfo)
	local k, l = nil
...
RegisterNetEvent('jim-shops:server:sellChips', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)  
    if not Player.Functions.GetItemByName("casinochips") then TriggerClientEvent('Roda_Notifications:showNotification', src, "You don't have any "..QBCore.Shared.Items["casinochips"].label.." to sell", 'error', 6000) return
	elseif Player.Functions.GetItemByName("casinochips") then
		...
		if Player.Functions.RemoveItem("casinochips", amount) then
			TriggerClientEvent('Roda_Notifications:showNotification', src, "You sold your chips for $"..price, 'error', 6000)
