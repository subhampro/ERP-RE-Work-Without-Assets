...
RegisterNetEvent('qb-occasions:server:ReturnVehicle', function(vehicleData)
    ...
    if result[1] then
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent('Roda_Notifications:showNotification', src, Lang:t('error.not_your_vehicle'), 'error', 6000)
        end
    else
        TriggerClientEvent('Roda_Notifications:showNotification', src, Lang:t('error.vehicle_does_not_exist'), 'error', 6000)
    end
end)
...
RegisterNetEvent('qb-occasions:server:sellVehicleBack', function(vehData)
    ...
    TriggerClientEvent('Roda_Notifications:showNotification', src, Lang:t('success.sold_car_for_price', { value = payout }), 'success', 6000)
...
RegisterNetEvent('qb-occasions:server:buyVehicle', function(vehicleData)
    ...
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            TriggerClientEvent('Roda_Notifications:showNotification', src, 'You dont have enough money', 'error', 6000)
        end
    end
end)
