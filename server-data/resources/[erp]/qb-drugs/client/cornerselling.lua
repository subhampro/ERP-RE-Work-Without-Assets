        ...
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    TriggerServerEvent('police:server:policeAlert', 'ALERT Drug Sales In Progress ::. Type: Drug Sales')
...
local function SellToPed(ped)
    ...
    local succesChance = math.random(1, 15)

    local scamChance = math.random(1, 10)

    local getRobbed = math.random(1, 25)
