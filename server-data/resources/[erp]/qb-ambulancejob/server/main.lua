...
RegisterNetEvent('hospital:server:Checkinbill', function(bedId, isRevive)
	local src = source
...
QBCore.Commands.Add("armor", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
			-- TriggerClientEvent('hospital:client:Revive', Player.PlayerData.source)
			TriggerServerEvent('hospital:server:SetArmor', 100, Player.PlayerData.source)
		else
			TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_online'), "error")
		end
	else
