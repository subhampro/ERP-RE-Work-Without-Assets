...
QBCore.Commands.Add("setcryptoworth", "Set crypto value", {{name="crypto", help="Name of the crypto currency"}, {name="Value", help="New value of the crypto currency"}}, false, function(source, args)
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', src, "This Crypto does not exist :(, available: ERP Token")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "You have not provided Crypto, available: ERP Token")
    end
end, "admin")

QBCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('QBCore:Notify', src, "The ERP Token has a value of: $"..Crypto.Worth["qbit"])
end)
...
QBCore.Commands.Add("crypto", "", {}, false, function(source)
    ...
    TriggerClientEvent('QBCore:Notify', src, "You have: "..Player.PlayerData.money.crypto.." ERP Token, with a value of: $"..MyPocket..",-")
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "You have exchanged your Cryptostick for: "..Amount.." ERP Token(\'s)", "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('qb-phone:client:AddTransaction', src, Player, {}, "There are "..Amount.." ERP Token('s) credited!", "Credit")
    end
end)
...
QBCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "You have "..tonumber(data.Coins).." ERP Token('s) purchased!", "Credit")
...
QBCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "You have "..tonumber(data.Coins).." ERP Token('s) sold!", "Depreciation")
...
QBCore.Functions.CreateCallback('qb-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
            TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "You have "..tonumber(data.Coins).." ERP Token('s) transferred!", "Depreciation")
            ...
            if Target ~= nil then
                ...
                TriggerClientEvent('qb-phone:client:AddTransaction', Target.PlayerData.source, Player, data, "There are "..tonumber(data.Coins).." ERP Token('s) credited!", "Credit")
            else
